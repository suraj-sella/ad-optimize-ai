...
class DataAnalyzerAgent extends BaseAgent {
  ...
  identifyPatterns(data) {
    ...
    const highCTR = rows.filter(row => row.calculated_ctr && row.calculated_ctr > 10);
    const highROAS = rows.filter(row => row.calculated_roas && row.calculated_roas > 3);
    return [
      { type: 'high_ctr', count: highCTR.length, keywords: highCTR.map(r => r.keyword) },
      { type: 'high_roas', count: highROAS.length, keywords: highROAS.map(r => r.keyword) }
    ];
  }
  ...
  identifyAnomalies(data) {
    ...
    const rows = data.rows || data;
    const zeroConversions = rows.filter(row => row.conversions === 0 || !row.conversions);
    const highACOS = rows.filter(row => row.calculated_acos && row.calculated_acos > 50);
    return [
      { type: 'zero_conversions', count: zeroConversions.length, keywords: zeroConversions.map(r => r.keyword) },
      { type: 'high_acos', count: highACOS.length, keywords: highACOS.map(r => r.keyword) }
    ];
  }
}
